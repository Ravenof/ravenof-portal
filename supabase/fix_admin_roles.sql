-- ════════════════════════════════════════════════════════════════
-- FIX: Admin roles RLS + is_admin() + is_moderator() functions
-- Safe: IF NOT EXISTS + CREATE OR REPLACE + DROP POLICY IF EXISTS
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tc.table_schema = 'public'
      ...
      AND cc.check_clause LIKE '%user%admin%event_moderator%'
  ) THEN
    ALTER TABLE public.profiles
      ADD CONSTRAINT profiles_role_check
      CHECK (role IN ('user', 'event_moderator', 'admin'));
  END IF;
END;
$$;

-- ────────────────────────────────────────────────────────────────
-- 2. is_admin() — used in RLS policies across tables
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 3. is_moderator() — admin OR event_moderator
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 4. profiles RLS — admin can SELECT all + UPDATE any profile
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 5. events RLS — re-apply using correct is_admin() / is_moderator()
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 6. event_registrations RLS — re-apply with correct functions
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 7. cards RLS — admin can manage all cards
-- ────────────────────────────────────────────────────────────────
...
SELECT
  (SELECT COUNT(*) FROM information_schema.columns
   WHERE table_schema='public' AND table_name='profiles' AND column_name='role') AS role_col,
  (SELECT COUNT(*) FROM information_schema.routines
   WHERE routine_schema='public' AND routine_name='is_admin') AS is_admin_fn,
  (SELECT COUNT(*) FROM information_schema.routines
   WHERE routine_schema='public' AND routine_name='is_moderator') AS is_moderator_fn,
  (SELECT COUNT(*) FROM pg_policies
   WHERE tablename='profiles' AND policyname='Admin update any profile') AS admin_update_policy,
  (SELECT COUNT(*) FROM storage.buckets
   WHERE id='avatars') AS avatars_bucket;
