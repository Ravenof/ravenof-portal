...
values
  ('shadowrook','ShadowRook','🪦',(select id from public.ranked_seasons where is_active limit 1),0,'mirties-marsas','Mirties maršas','control','low','high','high','low','hard','Ramus laiptų grindintojas, mėgsta lėtas kalades.'),
  ('lootgoblin69','LootGoblin69','👺',(select id from public.ranked_seasons where is_active limit 1),0,'vryhioko-gauja','Goblinų Gauja','aggro','very_high','low','low','very_high','normal','Chaotiškas goblinas, lekia į veidą ir lošia.'),
  ('silvertilt','SilverTilt','🛡️',(select id from public.ranked_seasons where is_active limit 1),0,'sviesos-pulkas','Šviesos pulkas','midrange','medium','medium','high','low','hard','Rimtas ranked žaidėjas, švarūs ėjimai.'),
  ('nomananocry','NoManaNoCry','🔮',(select id from public.ranked_seasons where is_active limit 1),0,'mistikos-melodija','Mistikos melodija','control','medium','high','medium','medium','hard','Burtų mėgėjas, kantrus value žaidimas.'),
  ('cursedealer','CurseDealer','🕯️',(select id from public.ranked_seasons where is_active limit 1),0,'demonu-orda','Demonų orda','curse','medium','high','medium','medium','hard','Erzinantis kontrolininkas, mėgsta trikdyti.'),
  ('piratewifi','PirateWifi','🏴‍☠️',(select id from public.ranked_seasons where is_active limit 1),0,'plesiku-naktis','Plėšikų naktis','tempo','high','medium','medium','medium','normal','Žaidžia greitai, baudžia klaidas.'),
  ('sneak-exe','Sneak.exe','🥷',(select id from public.ranked_seasons where is_active limit 1),0,'rytu-vejas','Rytų vėjas','stealth','medium','medium','low','low','hard','Asasinas, laukia lethal lango.'),
  ('healmepls','HealMePls','✨',(select id from public.ranked_seasons where is_active limit 1),0,'inkvizicijos-legionas','Inkvizicijos legionas','defensive','low','high','high','low','normal','Support mėgėjas, ilgi žaidimai.'),
  ('topdecktomas','TopdeckTomas','🍀',(select id from public.ranked_seasons where is_active limit 1),0,'plesiku-naktis','Plėšikų naktis','tempo','high','medium','medium','medium','normal','Laimingas laiptų žaidėjas, visad ištraukia.'),
  ('graveyardenjoyer','GraveyardEnjoyer','⚰️',(select id from public.ranked_seasons where is_active limit 1),0,'mirties-marsas','Mirties maršas','midrange','medium','medium','medium','medium','normal','Mėgsta kapinyno mechanikas ir recursion.'),
  ('tauntpolice','TauntPolice','🚧',(select id from public.ranked_seasons where is_active limit 1),0,'sviesos-pulkas','Šviesos pulkas','defensive','low','high','high','low','normal','Blokuoja viską, verčia tradinti.'),
  ('laggywizard','LaggyWizard','🧙',(select id from public.ranked_seasons where is_active limit 1),0,'mistikos-melodija','Mistikos melodija','control','high','medium','medium','medium','normal','Magas su sprogstamais ėjimais.'),
  ('pactenjoyer','PactEnjoyer','😈',(select id from public.ranked_seasons where is_active limit 1),0,'demonu-orda','Demonų orda','curse','medium','high','medium','medium','hard','Mėgsta dark pact / curse, sunku žaisti prieš.'),
  ('faceisplace','FaceIsPlace','💥',(select id from public.ranked_seasons where is_active limit 1),0,'vryhioko-gauja','Goblinų Gauja','aggro','very_high','very_low','very_low','very_high','normal','Klasikinis aggro, ignoruoja lentą.'),
  ('critkestas','CritKestas','🎯',(select id from public.ranked_seasons where is_active limit 1),0,'rytu-vejas','Rytų vėjas','stealth','medium','medium','low','low','hard','Burst žaidėjas, vienas švarus kill.'),
  ('buffdaddylt','BuffDaddyLT','💪',(select id from public.ranked_seasons where is_active limit 1),0,'inkvizicijos-legionas','Inkvizicijos legionas','defensive','low','high','high','low','normal','Mėgsta buff stacking ir apsaugotus padarus.'),
  ('deckgoblin','DeckGoblin','🎒',(select id from public.ranked_seasons where is_active limit 1),0,'plesiku-naktis','Plėšikų naktis','midrange','medium','medium','medium','medium','normal','Praktiškas laiptų žaidėjas, prisitaiko.'),
  ('whispermain','WhisperMain','🌑',(select id from public.ranked_seasons where is_active limit 1),0,'demonu-orda','Demonų orda','curse','medium','high','medium','medium','hard','Debuff/control main.'),
  ('afk-baron','AFK_Baron','🪑',(select id from public.ranked_seasons where is_active limit 1),0,null,'Universalus','midrange','medium','low','medium','low','easy','Casual žaidėjas, paprastas bet ne beviltiškas.'),
  ('rankgremlin','RankGremlin','👹',(select id from public.ranked_seasons where is_active limit 1),0,null,'Mišri kaladė','control','high','high','high','low','hard','Sweaty ranked grinder — stipriausias botas laiptuose.')
