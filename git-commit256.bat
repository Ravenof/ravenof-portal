...
git commit -m "feat(cinematics): PremiumCinematics - kino pop-up sistema (summon + Cempiono skill). Naujas card-cinematics bucket (WebM->MP4->poster, tik URL DB). Admin: Summon Cinematic sekcija + kiekvienam is 3 skillu atskiras kino (CinematicUpload widget, preview/replace/delete, validacija). VERTIKALUS (mobile portrait 9/16) remas + admin fokuso/kadravimo selektorius (object-position X/Y, spausk-tempk). Max trukme 4 s. Bendra eile (useCinematicQueue - be persidengimo, cap, dedupe), RavenofCinematicOverlay (themed remas, skip 0.5s, reduced-motion static, video cleanup). Trigeriai TutorialGame: summon (Legendinis/Cempionas) per play/champion event; skill per ability event (e.skillIndex) PO taikinio - nekeicia zaidimo busenos. Nustatymai: premium/summon/skill jungikliai. Deck posteriu preload, video lazy. tsc svarus."
git push
) > commit256.log 2>&1
