          ...
          <div key={a.key} className="px-3 py-2.5 rounded-lg" style={{ background: 'rgba(10,8,16,0.55)', border: '1px solid ' + (st.completed ? 'rgba(240,180,41,0.4)' : 'rgba(255,255,255,0.07)') }}>
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold truncate" style={{ color: 'var(--text-primary)', fontFamily: 'var(--rvn-font-display)' }}>{a.name}</p>
                <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{a.description} · {summarizePayload(a.reward)}</p>
              </div>
              {st.claimed ? (
