      ...
      <div className="relative w-[min(460px,96vw)] my-6" style={{ clipPath: oct(18), background: won ? 'rgba(240,180,41,0.55)' : 'rgba(239,68,68,0.5)', padding: 2.5 }}>
        <div className="px-5 py-7" style={{ clipPath: oct(17), background: `radial-gradient(120% 80% at 50% 0%, rgba(${won ? '240,180,41' : '239,68,68'},0.16), rgba(10,8,16,0.98) 60%), linear-gradient(160deg,#15101f,#0a0810)` }}>
          ...
          <p className="text-center text-xs font-semibold mb-1" style={{ color: result.rankChange === 'up' ? '#86efac' : result.rankChange === 'down' ? '#f87171' : 'var(--text-muted)' }}>
            {result.rankChange === 'up' ? `Rangas pakilo · ${formatRank(result.rankStepAfter)}` : result.rankChange === 'down' ? `Rangas nukrito · ${formatRank(result.rankStepAfter)}` : 'Rangas nepasikeitė'}
          </p>
          {isMaxRank(result.rankStepAfter) && won && <p className="text-center text-[11px] mb-2" style={{ color: 'var(--gold)' }}>Aukščiausias rangas pasiektas!</p>}
          {lossWarn && <p className="text-center text-[11px] mb-2" style={{ color: '#fbbf24' }}>⚠ Pralaimėjimai iki rango kritimo: 1/2</p>}
          ...
          {result.unlockedRewardKeys.length > 0 && (
            <p className="text-center text-[11px] mb-1" style={{ color: 'var(--gold)' }}>🎁 Atrakinta atlygių: {result.unlockedRewardKeys.map((k) => MILESTONE_BY_KEY.get(k)?.title ?? k).join(', ')}</p>
          )}
          {result.completedAchievementKeys.length > 0 && (
            <p className="text-center text-[11px] mb-2" style={{ color: '#86efac' }}>🏅 Pasiekimai: {result.completedAchievementKeys.map((k) => ACHIEVEMENT_BY_KEY.get(k)?.name ?? k).join(', ')}</p>
          )}

          {/* Statistika */}
          <div className="rounded-lg px-3 py-2 my-3" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.06)' }}>
            {stat('Sunaikinta / prarasta', `${stats.totalKills} / ${stats.totalDeaths}`)}
            {stat('Žala priešininkui', stats.damageDealtToEnemyPlayer)}
            {stat('Gauta žala', stats.damageTaken)}
            {stat('Burtai sužaisti', stats.spellsPlayed)}
          </div>

          <div className="grid grid-cols-2 gap-2 mt-4">
            <RButton onClick={onAgain} tone="gold">Žaisti dar kartą</RButton>
            <RButton onClick={onHome} tone="muted">Grįžti į reitingą</RButton>
            <RButton onClick={onLeaderboard} tone="accent">Žiūrėti topą</RButton>
            <RButton onClick={onRewards} tone="accent">Apdovanojimai</RButton>
          </div>
        </div>
      </div>
    </div>
