      ...
      {items.map((it) => {
        ...
        return (
          <button key={it.f} role="tab" aria-selected={on} onClick={() => pick(it.f)} className="ravenof-press flex items-center"
            style={{ gap: big ? 8 : 5, padding: big ? '9px 22px' : '4px 10px', border: 0, cursor: on ? 'default' : 'pointer',
              font: `800 ${big ? 11.5 : 9.5}px var(--ravenof-font-display)`, letterSpacing: big ? '0.14em' : '0.1em', textTransform: 'uppercase',
            ...
            {it.label}
            <span style={{ font: `600 ${big ? 8.5 : 7.5}px var(--ravenof-font-body)`, letterSpacing: '0.1em', padding: '1px 5px', border: '1px solid currentColor', borderRadius: 3, opacity: 0.85 }}>{it.tag}</span>
          </button>
        )
      })}
    </div>
...
export function ClassicBadge({ style }: { style?: React.CSSProperties }) {
  ...
  return (
    <span style={{ font: '800 8.5px var(--ravenof-font-body)', letterSpacing: '0.16em', padding: '3px 8px', border: `1px solid rgb(${CLASSIC_ACCENT})`, color: '#e9f1ff', background: 'rgba(10,15,24,0.85)',
