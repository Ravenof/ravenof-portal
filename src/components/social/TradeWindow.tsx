...
export function TradeWindow({ tradeId, onClose }: { tradeId: string; onClose: () => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if ((s.status === 'completed' || s.status === 'cancelled') && !closedRef.current) {
      ...
      if (s.status === 'completed') { playSuccess(); setNote('✓ Mainai įvyko!') } else { setNote('Mainai atšaukti.') }
  ...
  const Offer = ({ cards, mine }: { cards: string[]; mine: boolean }) => (
    <div className="grid grid-cols-4 gap-1.5 min-h-[80px] p-2 rounded-lg" style={{ background: 'rgba(0,0,0,0.3)', border: '1px dashed var(--bg-border)' }}>
      {cards.map((id, i) => { const c = cardOf(id); return (
        <button key={id + i} disabled={!mine} onClick={() => mine && removeAt(i)} className="relative rounded overflow-hidden disabled:cursor-default" style={{ aspectRatio: '5 / 7', border: `1px solid ${c?.rarityColor || '#555'}` }} title={mine ? 'Pašalinti' : c?.name}>
          <Art url={c?.imageUrl ?? null} name={c?.name ?? '?'} />
        </button>
      ) })}
      {cards.length === 0 && <span className="col-span-4 text-[10px] self-center text-center" style={{ color: 'var(--text-muted)' }}>tuščia</span>}
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-3" style={{ background: 'rgba(0,0,0,0.85)' }} onClick={cancel}>
      <div className="w-full max-w-lg rounded-2xl p-4" style={{ background: 'var(--bg-surface)', border: '1px solid var(--bg-border)' }} onClick={(e) => e.stopPropagation()}>
        <p className="text-base font-bold mb-2 text-center" style={{ fontFamily: 'var(--rvn-font-display)', color: 'var(--gold)' }}>🔄 Mainai su {theirName ?? '…'}</p>
        {!t || t.status === 'pending' ? (
          <p className="text-sm text-center py-6" style={{ color: 'var(--text-muted)' }}>{t?.status === 'pending' ? 'Laukiama, kol kitas priims…' : 'Kraunama…'}</p>
        ) : (
          <>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                <p className="text-[11px] font-bold mb-1" style={{ color: myConfirmed ? '#4ade80' : 'var(--text-secondary)' }}>Tavo siūlymas {myConfirmed ? '✓' : ''}</p>
                <Offer cards={myOffer} mine />
              </div>
              <div>
                <p className="text-[11px] font-bold mb-1" style={{ color: theirConfirmed ? '#4ade80' : 'var(--text-secondary)' }}>{theirName} siūlo {theirConfirmed ? '✓' : ''}</p>
                <Offer cards={theirOffer} mine={false} />
              </div>
            </div>

            <p className="text-[10px] mb-1" style={{ color: 'var(--text-muted)' }}>Tavo kolekcija (spustelėk, kad pridėtum):</p>
            <div className="grid grid-cols-6 sm:grid-cols-8 gap-1.5 max-h-40 overflow-y-auto mb-3">
              ...
              {coll.length === 0 && <span className="col-span-full text-[10px] text-center py-3" style={{ color: 'var(--text-muted)' }}>Kolekcija tuščia.</span>}
            </div>

            {note && <p className="text-xs text-center mb-2" style={{ color: 'var(--gold)' }}>{note}</p>}
            <div className="flex gap-2 justify-between">
              <button onClick={cancel} className="px-3 py-2 rounded-lg text-xs" style={{ color: '#fca5a5', border: '1px solid rgba(239,68,68,0.3)' }}>Atšaukti mainus</button>
              <button onClick={toggleConfirm} disabled={busy || t.status !== 'active'} className="px-5 py-2 rounded-lg text-sm font-bold disabled:opacity-50"
                ...
                {myConfirmed ? '✓ Patvirtinta (atšaukti)' : 'Patvirtinti'}
              </button>
            </div>
          </>
        )}
