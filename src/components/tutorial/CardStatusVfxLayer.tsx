  ...
  switch (id) {
    case 'shield': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="shield">
        <div data-idle style={{ position: 'absolute', inset: -2, borderRadius: 10, border: `1.5px solid ${t}55`, boxShadow: `inset 0 0 10px ${t}2e`, animation: 'svfxPulse 3.2s ease-in-out infinite' }} />
        {parts > 0 && <div data-idle style={{ position: 'absolute', inset: -2, borderRadius: 10, overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: 0, bottom: 0, width: '45%', background: `linear-gradient(105deg, transparent, ${t}22, transparent)`, animation: 'svfxSheen 4.5s linear infinite' }} />
        </div>}
      </div>)
    case 'burning': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="burning">
        <div data-idle style={{ position: 'absolute', left: -1, right: -1, bottom: -2, height: '38%', borderRadius: '0 0 9px 9px', background: `linear-gradient(0deg, ${t}66, ${t}22 55%, transparent)`, transformOrigin: 'bottom', animation: 'svfxFlicker 1.1s ease-in-out infinite' }} />
        {Array.from({ length: parts }).map((_, i) => (
          <span key={i} data-idle style={{ position: 'absolute', bottom: 4, left: `${25 + i * 45}%`, width: 3, height: 3, borderRadius: 2, background: '#fdba74', animation: `svfxRise 1.6s ease-out ${i * 0.7}s infinite` }} />
        ))}
      </div>)
    case 'frozen': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="frozen">
        <div data-idle style={{ position: 'absolute', inset: -2, borderRadius: 10, border: `2px solid ${t}88`, boxShadow: `inset 0 0 12px ${t}44, 0 0 8px ${t}33`, animation: 'svfxShimmer 3.6s ease-in-out infinite' }} />
        <span style={{ position: 'absolute', top: -3, left: -3, width: 9, height: 9, background: t, opacity: 0.85, clipPath: 'polygon(50% 0, 100% 50%, 50% 100%, 0 50%)' }} />
        <span style={{ position: 'absolute', bottom: -3, right: -3, width: 7, height: 7, background: t, opacity: 0.7, clipPath: 'polygon(50% 0, 100% 50%, 50% 100%, 0 50%)' }} />
        {parts > 1 && <span data-idle style={{ position: 'absolute', top: '30%', right: 3, width: 2.5, height: 2.5, borderRadius: 2, background: '#e0f2fe', animation: 'svfxRise 2.8s linear infinite reverse' }} />}
      </div>)
    case 'poisoned': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="poisoned">
        <div data-idle style={{ position: 'absolute', inset: -2, borderRadius: 10, border: `1.5px solid ${t}66`, boxShadow: `inset 0 -8px 12px ${t}2a`, animation: 'svfxPulse 2.8s ease-in-out infinite' }} />
        {Array.from({ length: parts }).map((_, i) => (
    ...
    case 'stunned': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="stunned">
        <div data-idle style={{ position: 'absolute', top: -10, left: '50%', width: 30, height: 30, marginLeft: -15, animation: 'svfxOrbit 3s linear infinite' }}>
          {[0, 1, 2].slice(0, Math.max(2, parts + 1)).map((i) => (
            <span key={i} style={{ position: 'absolute', top: i === 0 ? 0 : i === 1 ? 18 : 8, left: i === 0 ? 12 : i === 1 ? 2 : 24, width: 5, height: 5, background: t, clipPath: 'polygon(50% 0, 65% 35%, 100% 50%, 65% 65%, 50% 100%, 35% 65%, 0 50%, 35% 35%)', filter: `drop-shadow(0 0 3px ${t})` }} />
          ))}
        </div>
    ...
    case 'silenced': return (
      <div className="rvnSvfx" style={{ zIndex: 5 }} data-vfx-idle="silenced">
        <div style={{ position: 'absolute', inset: 0, borderRadius: 8, background: 'rgba(15,10,25,0.3)' }} />
...
function OneShot({ s, onDone, q }: { s: Shot; onDone: () => void; q: 'low' | 'medium' | 'high' }) {
  ...
  if (s.statusId === 'shield' && (s.type === 'destroy' || s.type === 'remove')) {
    return (
      <div {...common}>
  ...
  return (
    <div {...common}>
      {s.statusId === 'shield' && <>
        <div data-anim onAnimationEnd={fire} style={{ position: 'absolute', inset: -3, borderRadius: 11, border: `2px solid ${t}`, boxShadow: `0 0 16px ${t}88, inset 0 0 14px ${t}44`, animation: 'svfxRing 0.5s cubic-bezier(0.2,0.8,0.3,1) both' }} />
      ...
      {s.statusId === 'burning' && <>
        ...
        <div data-anim style={{ position: 'absolute', inset: -2, borderRadius: 10, background: `${t}33`, animation: 'svfxFlash 0.35s ease-out both' }} />
      </>}
